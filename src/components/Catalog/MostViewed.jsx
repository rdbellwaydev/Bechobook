...
const CatalogBooks = () => {
  ...
  const [totalPages, setTotalPages] = useState(1);
    ...
    //    try {
    //      console.log("Checking if book is already in cart...");
     
    //      // ✅ Fetch current cart items
    //      const cartResponse = await fetch("https://bb.bechobookscan.com/api/getCart", {
    ...
    //      const isAlreadyInCart = cartItems.some((item) => item.book_id === product.book_id);
  ...
  const fetchCatalogBooks = async () => {
    try {
      ...
      const response = await fetch("https://bb.bechobookscan.com/api/getBooksByCatalog?catalogs=most_viewed", {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  return (
    <>
      ...
      <div className="flex justify-center items-center min-h-screen pb-24 pt-10">
        ...
        <h2 className="text-center text-3xl border-b-4 w-fit mx-auto pb-2 rounded border-lime-600 mb-4">Most Viewed</h2>
          {loading ? (
